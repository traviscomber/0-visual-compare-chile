...
export async function GET() {
  try {
    ...
    console.error("[inapi-sync] list error", error)
    return NextResponse.json({ error: "No fue posible cargar las sincronizaciones INAPI." }, { status: 500, headers: PRIVATE_NO_STORE_HEADERS })
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const delayMs = typeof body?.delayMs === "number" && Number.isFinite(body.delayMs) ? Math.min(10_000, Math.max(250, Math.floor(body.delayMs))) : 400
    const startIndex = typeof body?.startIndex === "number" && Number.isFinite(body.startIndex) ? Math.max(0, Math.floor(body.startIndex)) : 0
    const maxJobs = typeof body?.maxJobs === "number" && Number.isFinite(body.maxJobs) ? Math.min(100, Math.max(1, Math.floor(body.maxJobs))) : undefined

    if (query.length > 160 || queries.some((item: string) => item.length > 160) || queries.length > 100) {
      return NextResponse.json({ error: "La solicitud excede los límites permitidos." }, { status: 400, headers: PRIVATE_NO_STORE_HEADERS })
    }
    if (!query && !queries.length && !preset) {
      return NextResponse.json({ error: "Debes indicar un query, una lista de queries o un preset para sincronizar desde INAPI." }, { status: 400, headers: PRIVATE_NO_STORE_HEADERS })
    }
    ...
    const result = preset || queries.length > 1
      ...
      : await runInapiSync({ query: query || queries[0], searchType, matchMode, initiatedBy: authorization.user.id })

    return NextResponse.json({ ok: true, ...result }, { status: 201, headers: PRIVATE_NO_STORE_HEADERS })
  } catch (error) {
    console.error("[inapi-sync] run error", error)
    return NextResponse.json({ error: "No fue posible ejecutar la sincronización INAPI." }, { status: 500, headers: PRIVATE_NO_STORE_HEADERS })
  }
}
