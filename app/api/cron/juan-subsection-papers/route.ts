...
} from "@/lib/intelligence/subsection-research-delta"
...
export async function GET(request: Request) {
  ...
  for (const row of rows) {
    ...
    const previousResearch = readStoredSubsectionResearchForDelta(snapshot.subsection_research)
    ...
    const delta = buildSubsectionDiscoveryDelta(previousResearch, topicResults, { topicLimit: TOPIC_LIMIT, maxNewPapers: MAX_DELTA_PAPERS })
  ...
  const response = {
    ...
    boundary: BOUNDARY,
    deltaBoundary: SUBSECTION_DELTA_BOUNDARY,
    products: results,
