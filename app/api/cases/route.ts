...
export async function GET() {
  ...
  const cases = (data ?? []).map((item) => ({ ...item, item_count: Array.isArray(item.case_items) ? Number(item.case_items[0]?.count ?? 0) : 0, case_items: undefined }))
...
export async function PATCH(request: Request) {
  ...
  if (body.status === "decided") {
    ...
    if ((count ?? 0) > 0) return NextResponse.json({ error: "Este caso todavía tiene revisiones pendientes. Resuélvelas antes de marcarlo como Decidido." }, { status: 409, headers: PRIVATE_NO_STORE_HEADERS })
  }
