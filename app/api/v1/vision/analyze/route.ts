  ...
  try {
    ...
    if (!auth.ok) {
      const headers = auth.reason === "quota_exceeded" && auth.quota_daily !== undefined && auth.quota_monthly !== undefined && auth.usage_today !== undefined && auth.usage_month !== undefined
        ...
        : NO_STORE_HEADERS
      return NextResponse.json({ error: auth.reason === "quota_exceeded" ? "Cuota excedida." : "API key inválida.", code: auth.reason }, { status: auth.reason === "quota_exceeded" ? 429 : 401, headers })
    }
