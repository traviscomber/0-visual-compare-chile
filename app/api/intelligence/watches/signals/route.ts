...
export async function GET() {
  ...
  const [brandWatchesResult, patentWatchesResult, technologyWatchesResult, brandEventsResult, patentEventsResult, technologyEventsResult] = await Promise.all([
    ...
    auth.supabase.from("patent_watches").select("id,query,is_active").eq("user_id", auth.user.id),
    ...
    auth.supabase.from("patent_alert_events").select("id,watch_id,title,application_number,applicants,ipc_codes,filing_date,detected_at,read_at").eq("user_id", auth.user.id).order("detected_at", { ascending: false }).limit(150),
  ...
  const signals: CommonSignal[] = [
    ...
    ...(patentEventsResult.data ?? []).flatMap(row => {
      ...
      return [{
        ...
        watchQuery: watch.query,
        source: "INAPI · Patentes",
        title: row.title,
        detail: [row.applicants, row.application_number ? `Solicitud ${row.application_number}` : null, Array.isArray(row.ipc_codes) && row.ipc_codes.length ? `IPC ${row.ipc_codes.slice(0, 4).join(", ")}` : null].filter(Boolean).join(" · ") || null,
        occurredAt: row.filing_date,
...
export async function POST() {
  ...
  if (failed) {
    console.error("[common-watch-signals:review]", failed)
    return NextResponse.json({ error: "No pudimos registrar la revisión completa." }, { status: 500, headers: PRIVATE_NO_STORE_HEADERS })
  }
...
function relevanceRank(value: CommonSignal["relevance"]) {
