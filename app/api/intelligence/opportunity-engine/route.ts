...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [{ data: binding }, { data: orgSearches }, { data: userSearches }, { data: watches }, { data: recommendations }] = await Promise.all([
      admin
      ...
      admin
        ...
        .select("watch_type,query,is_active,last_checked_at,last_reviewed_at,metadata,created_at")
        .eq("user_id", auth.user.id)
        .eq("is_active", true)
    ...
    const result = await runOpportunityEngine({
      ...
      userWatches: (watches ?? []) as Array<Record<string, unknown>>,
