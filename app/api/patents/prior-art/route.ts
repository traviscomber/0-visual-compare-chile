...
export async function GET(request: Request) {
  ...
  const parsed = QuerySchema.safeParse({ q: url.searchParams.get("q") ?? "", ipc: url.searchParams.get("ipc") || undefined, limit: url.searchParams.get("limit") ?? 30 })
  ...
  try {
    ...
    await auth.supabase.from("usage_logs").insert({ user_id: auth.user.id, organization_id: null, action: "patent.prior_art_review", metadata: { query: parsed.data.q, ipc: parsed.data.ipc || null, candidates: review.summary.total, close_review: review.summary.closeReview, strategy: review.searchStrategy, duration_ms: Date.now() - startedAt } })
