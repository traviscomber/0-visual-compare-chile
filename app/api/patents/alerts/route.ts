...
export async function GET() {
  ...
  const [{ data: watches, error: watchError }, { data: events, error: eventError }] = await Promise.all([
    auth.supabase.from("patent_watches").select("id,watch_type,query,is_active,last_checked_at,created_at").order("created_at", { ascending: false }),
    auth.supabase.from("patent_alert_events").select("id,watch_id,title,application_number,applicants,ipc_codes,filing_date,detected_at,read_at").order("detected_at", { ascending: false }).limit(100),
  ])
...
export async function POST(request: Request) {
  ...
  const { data, error } = await auth.supabase.from("patent_watches").upsert({ user_id: auth.user.id, watch_type: type, query, normalized_query: normalized, is_active: true, last_checked_at: new Date().toISOString(), updated_at: new Date().toISOString() }, { onConflict: "user_id,watch_type,normalized_query" }).select("id,watch_type,query,is_active,last_checked_at,created_at").single()
...
export async function PATCH(request: Request) {
  ...
  if (body.eventId) {
    const { error } = await auth.supabase.from("patent_alert_events").update({ read_at: new Date().toISOString() }).eq("id", body.eventId)
  ...
  const { error } = await auth.supabase.from("patent_watches").update({ is_active: body.active, last_checked_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", body.id)
...
export async function DELETE(request: Request) {
  ...
  const { error } = await auth.supabase.from("patent_watches").delete().eq("id", id)
