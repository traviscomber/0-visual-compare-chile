...
export default async function DecisionBriefPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="mx-auto w-full max-w-[1200px] px-4 py-9 sm:px-6 lg:px-8 lg:py-12 print:max-w-none print:bg-white print:px-0 print:py-0 print:text-black">
      ...
      <article className="border-y border-border py-8 print:border-black/20 print:text-black sm:py-10">
        <header className="border-b border-border pb-8 print:border-black/20">
          ...
          <h1 className="mt-6 text-4xl font-normal tracking-[-0.04em] text-foreground print:text-black sm:text-5xl">{caseRow.title}</h1>
          <p className="mt-4 max-w-3xl text-lg leading-8 text-muted-foreground print:text-neutral-700">{brief.question}</p>
        ...
        <section className="grid gap-8 border-b border-border py-8 print:border-black/20 lg:grid-cols-[1.15fr_0.85fr]">
          <div><SectionLabel>Resumen ejecutivo</SectionLabel><p className="mt-3 text-base leading-7 text-foreground print:text-black">{brief.executiveSummary}</p></div>
          <div className="border-l-2 border-primary/35 pl-5 print:border-black/40"><SectionLabel>Decisión</SectionLabel><p className="mt-3 text-sm leading-6 text-foreground print:text-black">{brief.decision}</p></div>
        </section>

        <section className="grid gap-10 border-b border-border py-8 print:border-black/20 lg:grid-cols-2">
        ...
        <section className="border-b border-border py-8 print:border-black/20">
          <div className="flex items-end justify-between gap-3"><div><SectionLabel>Evidencia clave</SectionLabel><h2 className="mt-2 text-xl font-normal tracking-[-0.02em] text-foreground print:text-black">Qué sostiene esta lectura</h2></div><span className="text-xs text-muted-foreground print:text-neutral-600">{brief.evidence.length} destacada{brief.evidence.length===1?"":"s"}</span></div>
          <div className="mt-5 divide-y divide-border border-y border-border print:divide-black/20 print:border-black/20">
            {brief.evidence.length ? brief.evidence.map((item, index) => (
              <div key={`${item.type}-${item.title}-${index}`} className="grid gap-3 py-4 sm:grid-cols-[auto_1fr_auto] sm:items-start">
                ...
                <div><p className="text-sm font-medium leading-6 text-foreground print:text-black">{item.title}</p>{item.subtitle && <p className="mt-1 text-xs leading-5 text-muted-foreground print:text-neutral-600">{item.subtitle}</p>}</div>
                <time className="text-xs text-muted-foreground print:text-neutral-600">{formatDate(item.createdAt)}</time>
              </div>
            )) : <p className="py-8 text-sm text-muted-foreground print:text-neutral-600">Este caso todavía no contiene evidencia vinculada.</p>}
          </div>
        </section>

        <section className="grid gap-10 py-8 lg:grid-cols-[0.85fr_1.15fr]">
          <div><SectionLabel>Cambios recientes</SectionLabel><div className="mt-4 divide-y divide-border border-y border-border print:divide-black/20 print:border-black/20">{brief.recentChanges.map((change, index) => <div key={`${change}-${index}`} className="flex gap-3 py-3 text-sm leading-6 text-foreground print:text-black"><CalendarClock className="mt-1 h-4 w-4 shrink-0 text-muted-foreground print:text-neutral-600" /><span>{change}</span></div>)}</div></div>
          <div className="border-l-2 border-border pl-5 print:border-black/30"><SectionLabel>Trazabilidad</SectionLabel><p className="mt-3 text-sm leading-6 text-muted-foreground print:text-neutral-700">Este brief se genera exclusivamente desde el caso, su evidencia vinculada y su línea de tiempo auditable. No agrega hechos externos ni sustituye revisión profesional. El expediente original conserva el detalle y la fuente de cada hallazgo.</p><div className="mt-4 flex items-center gap-2 text-xs text-muted-foreground print:text-neutral-600"><BriefcaseBusiness className="h-4 w-4" />Caso {caseRow.id}</div></div>
        </section>
      </article>
    </main>
  )
...
function Meta({ label, value }: { label: string; value: string }) {
  return <div className="border-b border-border py-4 sm:border-b-0 sm:border-r sm:px-4 first:pl-0 last:border-r-0 print:border-black/20"><p className="text-[10px] font-medium uppercase tracking-[0.13em] text-muted-foreground print:text-neutral-600">{label}</p><p className="mt-1 text-sm font-medium text-foreground print:text-black">{value}</p></div>
}
...
function BriefList({ title, icon: Icon, items }: { title: string; icon: typeof ShieldCheck; items: string[] }) {
  return <div><div className="flex items-center gap-2"><Icon className="h-4 w-4 text-primary print:text-black" /><h2 className="text-lg font-semibold text-foreground print:text-black">{title}</h2></div><div className="mt-4 divide-y divide-border border-y border-border print:divide-black/20 print:border-black/20">{items.length?items.map((item, index) => <p key={`${title}-${index}`} className="py-3 text-sm leading-6 text-foreground print:text-black">{item}</p>):<p className="py-3 text-sm text-muted-foreground print:text-neutral-600">Sin elementos registrados.</p>}</div></div>
}
