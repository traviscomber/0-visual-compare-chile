  ...
  return <main className="mx-auto w-full max-w-[1480px] px-4 py-9 sm:px-6 lg:px-8 lg:py-12">
    ...
    <header className="border-b border-border pb-8">
      <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
        <div className="max-w-4xl">
          ...
          {canEdit?<Input value={title} onChange={e=>{setTitle(e.target.value);setSaved(false)}} maxLength={160} aria-label="Nombre del caso" className="mt-3 h-auto border-0 bg-transparent px-0 text-3xl font-normal tracking-[-0.04em] shadow-none focus-visible:ring-0 sm:text-5xl"/>:<h1 className="mt-3 text-3xl font-normal tracking-[-0.04em] text-foreground sm:text-5xl">{title}</h1>}
        ...
        {canEdit?<div className="flex flex-wrap items-end gap-3">
          ...
          <div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Estado</p><p className="mt-2 text-sm text-foreground">{STATUS_LABELS[status]}</p></div>
          <div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Prioridad</p><p className="mt-2 text-sm text-foreground">{PRIORITY_LABELS[priority]}</p></div>
          <Badge variant="outline" className="rounded-md">Solo lectura</Badge>
        </div>}
      </div>
    </header>
    ...
    <section className="py-8">
      <div className="flex flex-col gap-4 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={()=>void load()}><RefreshCw className="mr-2 h-4 w-4"/>Actualizar</Button>{canEdit&&<Button variant="outline" onClick={()=>void markReviewed()} disabled={reviewing||intelligence.newEvidenceCount===0}>{reviewing?<Loader2 className="mr-2 h-4 w-4 animate-spin motion-reduce:animate-none"/>:<CheckCircle2 className="mr-2 h-4 w-4"/>}Marcar como revisado</Button>}</div>
      </div>
