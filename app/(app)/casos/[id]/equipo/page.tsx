...
export default function CaseCollaborationPage() {
  ...
  if(!data) return <div className="mx-auto max-w-[1480px] px-4 py-10 text-destructive sm:px-6 lg:px-8">{error||"No pudimos cargar el caso."}</div>
  ...
  return <OperationalPage>
    ...
    <OperationalHeader eyebrow="VIDENTIA / Caso / Equipo" title="Personas y trabajo del caso." description={<>Coordina participantes, acciones y conversación sin perder contexto. Los permisos se aplican por rol y el trabajo asignado alimenta los pendientes.</>} meta={<><span className="inline-flex items-center gap-1.5"><Users className="h-3.5 w-3.5"/>Tu rol: {ROLE_LABELS[data.currentUserRole]}</span><span>{data.members.length} participante{data.members.length===1?"":"s"}</span><span>{data.actions.filter(item=>item.status==="open").length} acción{data.actions.filter(item=>item.status==="open").length===1?"":"es"} abierta{data.actions.filter(item=>item.status==="open").length===1?"":"s"}</span></>}/>
    ...
    <section className="grid gap-8 border-b border-border/80 py-7 xl:grid-cols-[0.82fr_1.18fr]">
      <div>
        <OperationalSectionHeader eyebrow="01 / Colaboración" title="Participantes" meta="Responsable, editores y observadores con acceso explícito."/>
      ...
      <div>
        <OperationalSectionHeader eyebrow="02 / Colaboración" title="Acciones" meta="Trabajo concreto con responsable y fecha cuando corresponde."/>
        ...
        <div className="mt-4 divide-y divide-border border-y border-border">{data.actions.length===0?<p className="py-7 text-sm text-muted-foreground">No hay acciones registradas.</p>:data.actions.map(action=>{const assigned=action.assigned_to?memberMap.get(action.assigned_to):null;const canToggle=canAssign||action.assigned_to===data.currentUserId;const closing=closingActionId===action.id;return <div key={action.id} className="py-3.5"><div className="flex items-start gap-3"><button disabled={!canToggle||busy} onClick={()=>{if(action.status==="done")void request({type:"action",id:action.id,status:"open"},"PATCH");else{setClosingActionId(action.id);setOutcome("")}}} className="mt-0.5 rounded-sm outline-none focus-visible:ring-2 focus-visible:ring-primary/40 disabled:opacity-40" aria-label={action.status==="done"?"Reabrir acción":"Completar acción"}>{action.status==="done"?<CheckCircle2 className="h-5 w-5 text-primary"/>:<Circle className="h-5 w-5 text-muted-foreground"/>}</button><div className="min-w-0 flex-1"><p className={action.status==="done"?"text-sm text-muted-foreground line-through":"text-sm font-medium text-foreground"}>{action.title}</p><p className="mt-1 text-xs text-muted-foreground">{assigned?`Responsable: ${assigned.display_name}`:"Sin responsable"}{action.due_at?` · vence ${formatDate(action.due_at)}`:""}</p>{action.outcome?<div className="mt-3 border-l-2 border-[#96B5A6]/40 pl-3"><p className="text-[10px] font-medium uppercase tracking-[0.12em] text-[#96B5A6]">Resultado</p><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-foreground/85">{action.outcome}</p></div>:null}</div></div>{closing?<div className="ml-8 mt-3"><textarea value={outcome} onChange={event=>setOutcome(event.target.value)} maxLength={2000} rows={3} autoFocus placeholder="¿Qué se resolvió, decidió o descartó?" className="w-full border border-input bg-background p-3 text-sm leading-6 text-foreground outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20"/><div className="mt-2 flex justify-end gap-2"><Button type="button" size="sm" variant="ghost" onClick={()=>{setClosingActionId(null);setOutcome("")}}>Cancelar</Button><Button type="button" size="sm" disabled={busy||outcome.trim().length<2} onClick={()=>void finishAction(action.id)}>Guardar resultado</Button></div></div>:null}</div>})}</div>
      </div>
    </section>
