...
export default async function PortfolioPage(){
  ...
  return <OperationalPage>
    <OperationalHeader
      ...
      title={attention?"Qué necesita una decisión y dónde está detenida.":"El portafolio no tiene bloqueos pendientes."}
      description={<>Una vista ejecutiva de casos, aprobaciones y vencimientos. Los estados se derivan del expediente y de las rondas de revisión registradas, no de un score opaco.</>}
    ...
    <OperationalMetricRail>
      <OperationalMetric value={counts.waiting} label="Esperando aprobación" detail="Rondas abiertas sin quórum"/>
      <OperationalMetric value={counts.blocked} label="Bloqueados" detail="Revisión pidió cambios" tone="danger"/>
      <OperationalMetric value={counts.overdue} label="Vencidos" detail="Plazo agotado sin quórum" tone="warning"/>
      <OperationalMetric value={counts.ready} label="Listos o aprobados" detail="Con decisión preparada o quórum" tone="success"/>
    </OperationalMetricRail>

    <section className="grid gap-8 border-b border-border/80 py-9 xl:grid-cols-[minmax(0,1.35fr)_minmax(280px,0.65fr)] xl:gap-10">
      <div>
        <OperationalSectionHeader eyebrow="Requiere atención" title="Casos que necesitan una acción" meta={`${critical.length} visibles`} />
...
function CaseRowView({item}:{item:PortfolioItem}){return <Link href={`/casos/${item.caseRow.id}/revision`} className="group grid gap-3 px-2 py-5 outline-none transition-colors hover:bg-secondary/55 focus-visible:bg-secondary/55 sm:grid-cols-[1fr_auto] sm:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><ExecutiveBadge status={item.execStatus}/>{item.caseRow.priority==="high"?<span className="text-xs font-medium text-amber-200">Prioridad alta</span>:null}</div><p className="mt-3 truncate text-sm font-medium text-white">{item.caseRow.title}</p><p className="mt-1 text-xs text-muted-foreground">{item.gov?`${item.approvals}/${item.gov.required_approvals} aprobaciones · ${item.pendingCount} pendientes`:"Sin ronda activa"}</p></div><ArrowRight className="h-4 w-4 shrink-0 text-muted-foreground transition-transform group-hover:translate-x-1"/></Link>}
function EmptyState(){return <div className="py-9"><CheckCircle2 className="h-5 w-5 text-primary"/><p className="mt-3 text-sm font-medium text-white">Nada requiere atención ahora.</p><p className="mt-1 text-sm leading-6 text-muted-foreground">No hay bloqueos, vencimientos ni aprobaciones pendientes.</p></div>}
