...
export default async function PredictiveRiskPage(){
  ...
  return <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:py-12">
    ...
    <section className="grid gap-7 border-b border-border pb-9 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
      <div><Badge variant="secondary">Phase 19 · Predictive Decision Risk</Badge><h1 className="mt-4 max-w-4xl text-4xl font-semibold tracking-[-0.035em] sm:text-5xl lg:text-6xl">Detecta qué decisiones probablemente se van a trabar.</h1><p className="mt-5 max-w-3xl text-base leading-7 text-muted-foreground sm:text-lg">El radar combina deadlines, espera real, carga de revisores, prioridad, solicitudes de cambio previas y falta de movimiento. Cada score es explicable y no depende de un modelo opaco.</p></div>
    ...
    <section className="grid gap-5 py-9 lg:grid-cols-[1.25fr_0.75fr]">
      <Card><CardHeader><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-medium uppercase tracking-[0.14em] text-muted-foreground">Radar preventivo</p><CardTitle className="mt-2 text-xl">Intervenir antes del atraso</CardTitle></div><Radar className="h-5 w-5 text-muted-foreground"/></div></CardHeader><CardContent className="space-y-3">{elevated.length?elevated.slice(0,8).map(({caseRow,risk})=><Link key={caseRow.id} href={`/casos/${caseRow.id}/revision`} className="block rounded-xl border border-border p-4 transition-colors hover:bg-secondary/30"><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><RiskBadge level={risk.level}/><Badge variant="outline">Score {risk.score}</Badge></div><ArrowRight className="h-4 w-4 text-muted-foreground"/></div><p className="mt-3 text-sm font-medium">{caseRow.title}</p><p className="mt-2 text-xs leading-5 text-muted-foreground">{risk.reasons.join(" · ")||"Sin señales materiales de atraso."}</p><p className="mt-3 text-xs font-medium">{risk.recommendedAction}</p></Link>):<div className="rounded-xl border border-dashed p-8 text-center"><ShieldCheck className="mx-auto h-5 w-5"/><p className="mt-3 text-sm font-medium">No hay casos con riesgo alto o crítico.</p><p className="mt-1 text-sm text-muted-foreground">El radar no detecta señales fuertes de atraso en este momento.</p></div>}</CardContent></Card>
