  ...
  return (
    <nav className={`${sticky ? "sticky top-0" : "relative"} z-50 border-b border-[#20363E] bg-[#091A20]/95 backdrop-blur`} aria-label={locale === "es" ? "Navegación principal" : "Primary navigation"}>
      <div className="mx-auto flex h-20 max-w-[1480px] items-center justify-between gap-5 px-5 lg:px-10">
        <Link href={paths.home} className="shrink-0" aria-label="VIDENTIA home">
        ...
        <div className="hidden items-center gap-5 text-[10px] font-medium tracking-[0.07em] text-[#BDBEBD] md:flex">
          {navItems.map(([section, label, href]) => (
            <Link key={section} href={href} aria-current={active === section ? "page" : undefined} className={active === section ? "text-white" : "hover:text-white"}>{label}</Link>
          ))}
          <Link href={paths.pricing} className="hidden hover:text-white xl:inline">{text.pricing}</Link>
        ...
        <div className="flex items-center gap-3 md:hidden">
          ...
          <details className="group relative">
            ...
            <div className="absolute right-0 top-[calc(100%+12px)] w-[min(82vw,320px)] border border-[#294047] bg-[#091A20] p-2 shadow-2xl">
              {navItems.map(([section, label, href]) => (
                <Link key={section} href={href} aria-current={active === section ? "page" : undefined} className={`block border-b border-[#20363E] px-4 py-4 text-[11px] tracking-[0.08em] ${active === section ? "text-white" : "text-[#BDBEBD]"}`}>{label}</Link>
              ))}
              <Link href={paths.pricing} className="block border-b border-[#20363E] px-4 py-4 text-[11px] tracking-[0.08em] text-[#BDBEBD]">{text.pricing}</Link>
              <Link href={paths.resources} className="block border-b border-[#20363E] px-4 py-4 text-[11px] tracking-[0.08em] text-[#BDBEBD]">{text.resources}</Link>
              <Link href={paths.login} className="block border-b border-[#20363E] px-4 py-4 text-[11px] tracking-[0.08em] text-[#BDBEBD]" prefetch={false}>{text.login}</Link>
              <Link href={switcher.href} className="block px-4 py-4 text-[11px] tracking-[0.08em] text-[#96B5A6]">{switcher.label}</Link>
            </div>
          </details>
        </div>
      </div>
