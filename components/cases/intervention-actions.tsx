...
export function InterventionActions({ caseId, priority, hasActiveRound }: { caseId:string; priority:"low"|"normal"|"high"; hasActiveRound:boolean }) {
  ...
  const run=async(action:Action)=>{
    if(action==="extend_deadline"&&!window.confirm("¿Extender el deadline de la ronda actual en 2 días?"))return
    ...
    try{
      ...
      setMessage(action==="remind_reviewers"?(Number(p.notified)>0?`Recordatorio enviado a ${p.notified} revisor${Number(p.notified)===1?"":"es"}.`:"No se enviaron recordatorios nuevos; existe uno reciente."):action==="extend_deadline"?"Deadline extendido en 2 días.":"Prioridad elevada a alta.")
      window.setTimeout(()=>window.location.reload(),700)
    }catch(error){setMessage(error instanceof Error?error.message:"No pudimos ejecutar la intervención.")}
    finally{setBusy(null)}
  }

  return <div className="mt-4 rounded-xl border border-border bg-secondary/20 p-3">
    <p className="text-xs font-medium uppercase tracking-[0.12em] text-muted-foreground">Intervenciones</p>
    <div className="mt-3 flex flex-wrap gap-2">
      ...
      <Button asChild size="sm" variant="ghost"><Link href={`/casos/${caseId}/revision`}>Abrir gobernanza</Link></Button>
    </div>
    {message&&<p className="mt-3 text-xs leading-5 text-muted-foreground">{message}</p>}
  </div>
}

function ActionIcon({busy,icon:Icon}:{busy:boolean;icon:typeof BellRing}){return busy?<Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin"/>:<Icon className="mr-1.5 h-3.5 w-3.5"/>}
