...
const copy = {
  es: { back: "Volver a VIDENTIA", eyebrow: "Acceso", title: "Entra a tu espacio de trabajo.", body: "Investiga antecedentes, revisa evidencia, gestiona casos y vigila cambios desde un mismo sistema.", email: "Correo", emailPlaceholder: "nombre@empresa.cl", password: "Contraseña", recover: "Recuperar contraseña", submit: "Entrar", loading: "Entrando…", required: "Completa tu correo y contraseña.", failed: "No pudimos iniciar sesión. Revisa tus datos e inténtalo nuevamente.", need: "¿Necesitas acceso?", talk: "Solicitar acceso empresarial", side: "Busca. Entiende. Decide. Vigila.", principles: [["Fuente visible", "El origen y la disponibilidad de la evidencia no se ocultan."], ["Señales separadas", "La prioridad organiza revisión; no reemplaza el juicio jurídico."], ["Trazabilidad", "Casos, decisiones y cambios conservan contexto para volver a revisar."]] },
  en: { back: "Back to VIDENTIA", eyebrow: "Access", title: "Enter your workspace.", body: "Research prior rights, review evidence, manage cases and monitor changes from one system.", email: "Email", emailPlaceholder: "name@company.com", password: "Password", recover: "Recover password", submit: "Sign in", loading: "Signing in…", required: "Enter your email and password.", failed: "We could not sign you in. Check your details and try again.", need: "Need access?", talk: "Request enterprise access", side: "Search. Understand. Decide. Monitor.", principles: [["Visible source", "Evidence origin and availability remain visible."], ["Separated signals", "Priority organizes review; it does not replace legal judgment."], ["Traceability", "Cases, decisions and changes preserve context for future review."]] },
} as const
  ...
  return (
    <main className="min-h-svh bg-background text-foreground">
      <div className="mx-auto grid min-h-svh w-full max-w-[1480px] lg:grid-cols-[0.92fr_1.08fr]">
        ...
        <aside className="hidden min-h-svh flex-col justify-between bg-card/20 px-12 py-10 lg:flex"><div className="flex justify-end"><span className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">VIDENTIA</span></div><div className="max-w-xl pb-12"><p className="font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-primary">VIDENTIA / trademark intelligence</p><p className="mt-5 text-3xl font-medium leading-tight tracking-[-0.035em] text-foreground">{t.side}</p><div className="mt-8 divide-y divide-border border-y border-border">{t.principles.map(([title, text], index) => <div key={title} className="grid grid-cols-[36px_1fr] gap-4 py-4"><span className="font-mono text-[10px] text-primary">0{index + 1}</span><div><p className="text-sm font-medium text-foreground">{title}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{text}</p></div></div>)}</div></div><p className="text-xs leading-5 text-muted-foreground">VIDENTIA · by N3uralia</p></aside>
      </div>
    </main>
  )
}
