create index if not exists patent_records_title_normalized_trgm_idx
  on public.patent_records using gin (public.normalize_inapi_search_text(title) gin_trgm_ops);

create index if not exists patent_records_applicants_normalized_trgm_idx
  on public.patent_records using gin (public.normalize_inapi_search_text(applicants) gin_trgm_ops);

create index if not exists patent_record_ipc_code_pattern_idx
  on public.patent_record_ipc (code text_pattern_ops, patent_record_id);
...
as $function$
  with params as (
    ...
    select pr.*,
           similarity(public.normalize_inapi_search_text(coalesce(pr.title, '')), params.q)::real as title_similarity,
           similarity(public.normalize_inapi_search_text(coalesce(pr.applicants, '')), params.q)::real as applicant_similarity
    from public.patent_records pr
    cross join params
    where params.q <> ''
      and (
        public.normalize_inapi_search_text(pr.title) % params.q
        or public.normalize_inapi_search_text(pr.applicants) % params.q
        or public.normalize_inapi_search_text(pr.title) like '%' || params.q || '%'
        or public.normalize_inapi_search_text(pr.applicants) like '%' || params.q || '%'
      )
    order by greatest(
      similarity(public.normalize_inapi_search_text(coalesce(pr.title, '')), params.q),
      similarity(public.normalize_inapi_search_text(coalesce(pr.applicants, '')), params.q)
    ) desc
    limit 250
  )
  select c.id,
