-- Keep freshness_policy machine-readable. Preserve previous descriptive cadence in metadata.
update public.intelligence_sources
set metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
      'freshness_description', freshness_policy
    ),
    freshness_policy = 'bajo_demanda',
    updated_at = now()
where source_key = 'gleif'
  and freshness_policy is distinct from 'bajo_demanda';
...
set metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
    ...
    freshness_policy = 'cada_15_min',
    updated_at = now()
where source_key in ('gdelt_raw_feed', 'gdelt_mentions', 'gdelt_gkg')
  and freshness_policy is distinct from 'cada_15_min';
...
begin
  ...
  loop
    ...
    v_sla_hours := case v_policy
      when 'cada_15_min' then 1
      when 'diaria' then 36
