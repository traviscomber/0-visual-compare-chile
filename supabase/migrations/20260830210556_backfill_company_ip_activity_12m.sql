create or replace function public.split_company_applicants(value text)
returns table(applicant text)
language sql
immutable
parallel safe
...
with raw_aliases as (
  select
    ...
    coalesce(public.company_country_hint(part.applicant), nullif(upper(e.country), '')) as country_hint,
    'intelligence_graph'::text as source_scope,
    2 as source_priority,
    e.first_seen_at,
    e.last_seen_at
  from public.intelligence_entities e
  cross join lateral public.split_company_applicants(e.canonical_name) part
...
select
  ...
  tr.estado,
  coalesce((select array_agg(distinct n.code order by n.code) from public.trademark_record_niza n where n.trademark_record_id = tr.id), '{}'::text[]),
  tr.source_url,
...
with patent_applicants as (
  select pr.*, part.applicant as applicant_raw
  from public.patent_records pr
  cross join lateral public.split_company_applicants(pr.applicants) part
  where pr.filing_date >= current_date - 365
    and public.normalize_company_identity(part.applicant) is not null
...
select
  ...
  pr.filing_date,
  pr.status,
  coalesce((select array_agg(distinct p.code order by p.code) from public.patent_record_ipc p where p.patent_record_id = pr.id), '{}'::text[]),
  pr.source_url,
  jsonb_build_object('backfill', '12m'),
