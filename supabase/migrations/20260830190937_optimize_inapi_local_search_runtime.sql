create or replace function public.search_inapi_local(p_query text, p_niza_codes text[] default '{}'::text[], p_limit integer default 20)
returns table(
  id uuid,
  nombre text,
  solicitante text,
...
as $function$
  with params as (
    ...
    select tr.*,
           ...
           public.normalize_inapi_search_text(tr.nombre)=params.q exact_match
    from public.trademark_records tr
    cross join params
    where tr.source='inapi'
      and params.q<>''
      and coalesce(tr.nombre,'')<>''
      and (
        public.normalize_inapi_search_text(tr.nombre)=params.q
        or public.normalize_inapi_search_text(tr.nombre) % params.q
    ...
    from candidates c
    left join public.trademark_record_niza n on n.trademark_record_id=c.id
