...
begin
  ...
  v_assigned_to := coalesce(p_assigned_to, v_user_id);
  ...
  if v_case_id is null then
    insert into public.cases (
      user_id, title, status, priority, context_type, context_query, updated_at
    ) values (
      v_user_id, trim(p_case_title), 'open', p_priority, p_context_type, v_context_query, pg_catalog.now()
    )
  ...
  if v_item_id is null then
    insert into public.case_items (
      case_id, item_type, source_id, title, metadata
    ) values (
      v_case_id, p_item_type, trim(p_source_id), trim(p_source_title), coalesce(p_evidence, '{}'::jsonb)
    )
  ...
  if v_action_id is null then
    insert into public.case_actions (
      case_id, title, assigned_to, created_by, status, due_at, updated_at
    ) values (
      v_case_id, trim(p_action_title), v_assigned_to, v_user_id, 'open', p_due_at, pg_catalog.now()
    )
...
revoke all on function public.create_intelligence_action(text,text,text,text,text,text,text,text,timestamptz,uuid,jsonb) from public, anon;
grant execute on function public.create_intelligence_action(text,text,text,text,text,text,text,text,timestamptz,uuid,jsonb) to authenticated;
