...
create or replace function public.normalize_inapi_search_text(value text)
returns text
language sql
immutable
parallel safe
...
as $$ select upper(extensions.unaccent(value)) $$;

create index if not exists trademark_records_nombre_normalized_trgm_idx
  on public.trademark_records using gin (public.normalize_inapi_search_text(nombre) gin_trgm_ops)
  where source='inapi';
...
as $$
  with params as (
    ...
    where tr.source='inapi' and params.q<>'' and (
