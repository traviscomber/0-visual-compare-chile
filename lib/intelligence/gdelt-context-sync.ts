...
type Result=Counts&{artifactId:string;skipped:boolean;sha256:string|null}
...
export async function syncGdeltContextBundle(admin:SupabaseClient):Promise<{ok:true;timestamp:string;mentions:Result;gkg:Result}> {
  ...
  if(bundle.mentions.timestamp!==bundle.gkg.timestamp) throw new Error("GDELT context timestamps are not synchronized")
...
async function probeBundle(){
  ...
  return {mentions:artifact(lines.find(x=>x.endsWith(".mentions.CSV.zip")),"mentions"),gkg:artifact(lines.find(x=>x.endsWith(".gkg.csv.zip")),"gkg")}
}
...
async function syncMentions(admin:SupabaseClient,a:Artifact){
  ...
  try{
    ...
    if(!c.claimed){await finishIntelligenceIngestion(admin,{runId:ing.runId,sourceId:ing.sourceId,metadata:{pipeline:"gdelt-context-fusion",artifactKind:"mentions",skipped:c.status,artifactId:id}});return {...zero(id,true),identifiers:new Set<string>()}}
    ...
    return {artifactId:id,skipped:false,sha256:d.sha256,...counts,identifiers}
...
async function syncGkg(admin:SupabaseClient,a:Artifact,ids:Set<string>):Promise<Result>{
  ...
  try{
    const c=await claim(admin,a,ing.runId); id=c.artifactId
    if(!c.claimed){await finishIntelligenceIngestion(admin,{runId:ing.runId,sourceId:ing.sourceId,metadata:{pipeline:"gdelt-context-fusion",artifactKind:"gkg",skipped:c.status,artifactId:id}});return zero(id,true)}
    ...
    return {artifactId:id,skipped:false,sha256:d.sha256,...counts}
...
function zero(id:string,skipped:boolean):Result{return{artifactId:id,skipped,sha256:null,rowCount:0,inserted:0,updated:0,rejected:0,filtered:0}}
