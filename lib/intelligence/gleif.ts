...
type GleifRecord = { id?: string; attributes?: { lei?: string; entity?: { legalName?: { name?: string }; legalAddress?: { country?: string }; status?: string }; registration?: { status?: string } } }
...
export async function resolveLegalEntityInGleif(name: string, countryHint?: string | null): Promise<GleifLegalEntityMatch | null> {
  ...
  const response = await fetchWithRetry(`${GLEIF_BASE}/lei-records?${params.toString()}`, { cache: "no-store", headers: { Accept: "application/vnd.api+json", "User-Agent": "VIDENTIA/1.0" } }, { attempts: 1, baseDelayMs: 0, timeoutMs: TIMEOUT_MS })
...
function toCandidate(record: GleifRecord): GleifLegalEntityMatch | null {
  ...
  return { lei, legalName, country: normalizeCountry(record.attributes?.entity?.legalAddress?.country), registrationStatus: textOrNull(record.attributes?.registration?.status), entityStatus: textOrNull(record.attributes?.entity?.status), sourceUrl: `https://api.gleif.org/api/v1/lei-records/${encodeURIComponent(lei)}`, resolution: "normalized_exact", confidence: 1 }
}
function normalizeLegalName(value: string) { return value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase().replace(/\b(INC|INCORPORATED|CORP|CORPORATION|CO|COMPANY|LLC|LTD|LIMITED|PLC|S\.A\.|S A|SPA|SP A|LTDA|LIMITADA|EIRL|SAS|SOCIEDAD POR ACCIONES)\b/g, " ").replace(/[^A-Z0-9]+/g, " ").trim().replace(/\s+/g, " ") }
function normalizeCountry(value: string | null | undefined) { const text = String(value ?? "").trim().toUpperCase(); return /^[A-Z]{2}$/.test(text) ? text : null }
