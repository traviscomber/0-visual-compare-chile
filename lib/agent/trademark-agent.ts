...
export interface TrademarkInsightReport {
  ...
  registrabilidad?: {
    ...
    antecedentes: Array<{ id: string; nombre: string; solicitante: string; estado: string; clases: string[]; numero_registro: string; numero_solicitud: string; puntaje_relevancia: number; razones: string[]; similitud_denominativa: number; similitud_fonetica: number; imagen_url?: string }>
    ...
    calidad: { confianza: "alta" | "media" | "baja"; cobertura_clases: number; resultados_totales: number; resultados_activos: number; advertencias: string[]; estrategias_ejecutadas: number; estrategias: Array<{ id: string; label: string; query: string }>; resultados_brutos: number; duplicados_eliminados: number }
  }
...
export class TrademarkAgent {
  ...
  async analyze(req: TrademarkAgentRequest): Promise<TrademarkInsightReport> {
    ...
    try { registrabilidad = await this.searchInapiAvailability(req.nombreMarca, niza.clases) } catch (error) { console.error("[trademark-agent] INAPI unavailable", error); registrabilidad = buildUnavailableInapiResult(req.nombreMarca) }
  ...
  private async generateReport(params: { nombreMarca: string; viena: VienaClassification; niza: NizaClassification; conflictos: ConflictReport; registrabilidad?: TrademarkInsightReport["registrabilidad"]; visualScore?: number; tier: ModelTier }) {
    ...
    const antecedentesInapi = registrabilidad?.antecedentes.slice(0, 5).map((item) => `• ${item.nombre} — ${item.estado} — clases ${item.clases.join(", ") || "sin clase"} — relevancia ${item.puntaje_relevancia}/100 — denominativa ${item.similitud_denominativa}% — fonética ${item.similitud_fonetica}% — ${item.razones.join("; ")}`).join("\n") ?? "Fuente INAPI no disponible"
    const userPrompt = `Genera un informe ejecutivo interno para MARCA: "${nombreMarca}". SCORE VISUAL: ${visualScore ?? "No disponible"}/100. RIESGO DEL MOTOR: ${conflictos.nivel_riesgo_global.toUpperCase()}. Decisión operativa INAPI: ${registrabilidad?.decision ?? "FUENTE_NO_DISPONIBLE"}. Confianza: ${registrabilidad?.calidad.confianza ?? "baja"}. Estrategias ejecutadas: ${registrabilidad?.calidad.estrategias_ejecutadas ?? 0}. Resultados activos: ${registrabilidad?.calidad.resultados_activos ?? 0}. ANTECEDENTES PRIORIZADOS:\n${antecedentesInapi}\nCLASIFICACIÓN VIENA: ${viena.codes.slice(0, 5).map((c) => `${c.code}: ${c.elemento}`).join("; ") || "No determinada"}. CLASIFICACIÓN NIZA: ${niza.clases.map((c) => `Clase ${c.numero}: ${c.titulo}`).join("; ") || "No determinada"}. Entrega resumen máximo 3 oraciones, análisis, nivel de riesgo, hasta 5 recomendaciones y 4 próximos pasos.`
  ...
  private async searchInapiAvailability(nombreMarca: string, nizaClases: NizaClassification["clases"]): Promise<TrademarkInsightReport["registrabilidad"]> {
