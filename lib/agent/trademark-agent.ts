...
export interface TrademarkInsightReport {
  ...
  registrabilidad?: {
    ...
    antecedentes: Array<{ id: string; nombre: string; solicitante: string; estado: string; clases: string[]; numero_registro: string; numero_solicitud: string; puntaje_relevancia: number; razones: string[] }>
    ...
    calidad: { confianza: "alta" | "media" | "baja"; cobertura_clases: number; resultados_totales: number; resultados_activos: number; advertencias: string[]; estrategias_ejecutadas: number; resultados_brutos: number; duplicados_eliminados: number }
  }
...
export class TrademarkAgent {
  ...
  private async generateReport(params: { nombreMarca: string; viena: VienaClassification; niza: NizaClassification; conflictos: ConflictReport; registrabilidad?: TrademarkInsightReport["registrabilidad"]; visualScore?: number; tier: ModelTier }) {
    ...
    const antecedentesInapi = registrabilidad?.antecedentes.slice(0, 5).map((item) => `• ${item.nombre} — ${item.estado} — clases ${item.clases.join(", ") || "sin clase"} — relevancia ${item.puntaje_relevancia}/100 — ${item.razones.join("; ")}`).join("\n") ?? "Fuente INAPI no disponible"
  ...
  private async searchInapiAvailability(nombreMarca: string, nizaClases: NizaClassification["clases"]): Promise<TrademarkInsightReport["registrabilidad"]> {
    ...
    const antecedentes = ranked.slice(0, 12).map(({ marca, score, reasons }) => ({ id: marca.id, nombre: marca.nombre, solicitante: marca.solicitante, estado: marca.estado, clases: marca.niza, numero_registro: marca.numeroRegistro, numero_solicitud: String(marca.metadata?.numSolicitud ?? ""), puntaje_relevancia: score, razones }))
    const base = { antecedentes, fuente: { nombre: "INAPI" as const, modo: "consulta-live" as const, consulta: nombreMarca, tipo: "nombre" as const, match: "inteligente" as const, consultado_en: consultedAt }, calidad: { confianza: confidence, cobertura_clases: coverage, resultados_totales: execution.deduplicatedResultCount, resultados_activos: active.length, advertencias: warnings, estrategias_ejecutadas: execution.completedStrategies.length, resultados_brutos: execution.rawResultCount, duplicados_eliminados: Math.max(0, execution.rawResultCount - execution.deduplicatedResultCount) } }
