...
import { compareTrademarkCandidateImages } from "@/lib/image/trademark-visual-similarity"
...
export interface TrademarkInsightReport {
  ...
  viena: VienaClassification; niza: NizaClassification; conflictos: ConflictReport
  registrabilidad?: {
    ...
    antecedentes: Array<{ id: string; nombre: string; solicitante: string; estado: string; clases: string[]; numero_registro: string; numero_solicitud: string; puntaje_relevancia: number; razones: string[]; similitud_denominativa: number; similitud_fonetica: number; similitud_visual: number | null; imagen_url?: string }>
    ...
    calidad: { confianza: "alta" | "media" | "baja"; cobertura_clases: number; resultados_totales: number; resultados_activos: number; advertencias: string[]; estrategias_ejecutadas: number; estrategias: Array<{ id: string; label: string; query: string }>; resultados_brutos: number; duplicados_eliminados: number; imagenes_comparadas: number }
  }
...
const REPORT_SYSTEM_PROMPT = `Eres un analista interno senior de propiedad intelectual para una plataforma chilena de apoyo a decisiones. Redacta para CEO, administración y analistas. Sé directo, corporativo y verificable. Nunca declares que una marca es registrable, disponible, aprobada o rechazada jurídicamente. Distingue datos INAPI, inferencias del motor y limitaciones. No reemplazas asesoría jurídica.`
...
export class TrademarkAgent {
  ...
  async analyze(req: TrademarkAgentRequest): Promise<TrademarkInsightReport> {
    ...
    const [viena, niza] = await Promise.all([this.vienaClassifier.classify(req.imageBase64, req.imageMimeType), this.nizaClassifier.classify({ nombre: req.nombreMarca, descripcion: req.descripcion, industria: req.industria })])
    ...
    try { registrabilidad = await this.searchInapiAvailability(req.nombreMarca, niza.clases, req.imageBase64) } catch (error) { console.error("[trademark-agent] INAPI unavailable", error); registrabilidad = buildUnavailableInapiResult(req.nombreMarca) }
    ...
    return { marca: req.nombreMarca, timestamp: new Date().toISOString(), costo_estimado_usd, tokens_totales, routing: { max_tier_used: maxTier(maxTier(viena.routing.final_tier, niza.routing.final_tier), informe.routing.final_tier), escalated: viena.routing.escalated || niza.routing.escalated || reportTier !== "luna", report: informe.routing }, viena, niza, conflictos, registrabilidad, informe: informe.data, pipeline_ms: Date.now() - start }
  }
  ...
  private async generateReport(params: { nombreMarca: string; viena: VienaClassification; niza: NizaClassification; conflictos: ConflictReport; registrabilidad?: TrademarkInsightReport["registrabilidad"]; visualScore?: number; tier: ModelTier }) {
    ...
    const antecedentesInapi = registrabilidad?.antecedentes.slice(0, 5).map((item) => `• ${item.nombre} — ${item.estado} — clases ${item.clases.join(", ") || "sin clase"} — relevancia ${item.puntaje_relevancia}/100 — denominativa ${item.similitud_denominativa}% — fonética ${item.similitud_fonetica}%${item.similitud_visual != null ? ` — visual ${item.similitud_visual}%` : ""} — ${item.razones.join("; ")}`).join("\n") ?? "Fuente INAPI no disponible"
    const userPrompt = `Genera un informe ejecutivo interno para MARCA: "${nombreMarca}". SCORE VISUAL EXTERNO: ${visualScore ?? "No disponible"}/100. RIESGO DEL MOTOR: ${conflictos.nivel_riesgo_global.toUpperCase()}. Decisión operativa INAPI: ${registrabilidad?.decision ?? "FUENTE_NO_DISPONIBLE"}. Confianza: ${registrabilidad?.calidad.confianza ?? "baja"}. Estrategias ejecutadas: ${registrabilidad?.calidad.estrategias_ejecutadas ?? 0}. Imágenes comparadas: ${registrabilidad?.calidad.imagenes_comparadas ?? 0}. Resultados activos: ${registrabilidad?.calidad.resultados_activos ?? 0}. ANTECEDENTES PRIORIZADOS:\n${antecedentesInapi}\nCLASIFICACIÓN VIENA: ${viena.codes.slice(0, 5).map((c) => `${c.code}: ${c.elemento}`).join("; ") || "No determinada"}. CLASIFICACIÓN NIZA: ${niza.clases.map((c) => `Clase ${c.numero}: ${c.titulo}`).join("; ") || "No determinada"}. Entrega resumen máximo 3 oraciones, análisis, nivel de riesgo, hasta 5 recomendaciones y 4 próximos pasos.`
  ...
  private async searchInapiAvailability(nombreMarca: string, nizaClases: NizaClassification["clases"], imageBase64?: string): Promise<TrademarkInsightReport["registrabilidad"]> {
    ...
    const visualSignals = await compareTrademarkCandidateImages(imageBase64, ranked.slice(0, 6).map(({ marca }) => marca))
    ...
    if (imageBase64 && visualSignals.size === 0) warnings.push("La consulta incluyó imagen, pero los antecedentes priorizados no expusieron imágenes comparables en la fuente disponible.")
    const antecedentes = ranked.slice(0, 12).map(({ marca, score, reasons, denominativeSimilarity, phoneticSimilarity }) => {
      const visual = visualSignals.get(marca.id)
      const enrichedReasons = visual && visual.similarity >= 70 ? [...reasons, `similitud visual ${visual.similarity}%`] : reasons
      const relevance = visual ? Math.min(100, Math.round(score * 0.8 + visual.similarity * 0.2)) : score
      return { id: marca.id, nombre: marca.nombre, solicitante: marca.solicitante, estado: marca.estado, clases: marca.niza, numero_registro: marca.numeroRegistro, numero_solicitud: String(marca.metadata?.numSolicitud ?? ""), puntaje_relevancia: relevance, razones: enrichedReasons, similitud_denominativa: denominativeSimilarity, similitud_fonetica: phoneticSimilarity, similitud_visual: visual?.similarity ?? null, ...(visual?.imageUrl || marca.imagenUrl ? { imagen_url: visual?.imageUrl ?? marca.imagenUrl } : {}) }
    })
    const base = { antecedentes, fuente: { nombre: "INAPI" as const, modo: "consulta-live" as const, consulta: nombreMarca, tipo: "nombre" as const, match: "inteligente" as const, consultado_en: consultedAt }, calidad: { confianza: confidence, cobertura_clases: coverage, resultados_totales: execution.deduplicatedResultCount, resultados_activos: active.length, advertencias: warnings, estrategias_ejecutadas: execution.completedStrategies.length, estrategias: execution.completedStrategies.map(({ id, label, query }) => ({ id, label, query })), resultados_brutos: execution.rawResultCount, duplicados_eliminados: Math.max(0, execution.rawResultCount - execution.deduplicatedResultCount), imagenes_comparadas: visualSignals.size } }
    if (relevantActive.length > 0 && top) return { ...base, disponible: false, decision: "REVISAR", marca_encontrada: toPrimaryReference(top.marca), conflictos_reales: relevantActive.length, recomendacion: sameClassActive.length > 0 ? `Revisión prioritaria: existen ${sameClassActive.length} antecedentes activos en clases Niza relacionadas, incluyendo semejanzas denominativas, fonéticas y visuales cuando la fuente expone imágenes.` : `Revisión necesaria: existen ${active.length} antecedentes activos recuperados por la búsqueda inteligente.` }
    return { ...base, disponible: true, decision: "SIN_ANTECEDENTES_ACTIVOS", marca_encontrada: top ? toPrimaryReference(top.marca) : undefined, conflictos_reales: 0, recomendacion: ranked.length > 0 ? "No se detectaron antecedentes activos entre los resultados priorizados; conserve los históricos como contexto." : "No se detectaron coincidencias en las estrategias ejecutadas. Esto no garantiza registrabilidad." }
  }
}
