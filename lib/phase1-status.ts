...
export async function getPhase1StatusSummary(organizationId: string): Promise<Phase1StatusSummary> {
  ...
  if (!keys) throw new Error("No fue posible cargar las credenciales API para el estado Phase1.")
  if (recordsResponse.error) throw recordsResponse.error
  if (completedRunsResponse.error) throw completedRunsResponse.error
  if (failedRunsResponse.error) throw failedRunsResponse.error
