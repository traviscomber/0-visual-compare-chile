...
export async function syncCurrentYearInapiOpenData(year = new Date().getUTCFullYear()): Promise<OpenDataSyncSummary> {
  ...
  for (const dataset of DATASETS) {
    datasets.push(await syncDataset(dataset, year))
  }
...
async function syncDataset(dataset: (typeof DATASETS)[number], year: number) {
  ...
  try {
      ...
      const { error: progressError } = await admin
        ...
        .update({
          ...
          metadata: {
            ...
            trigger: "vercel-cron",
            progress: { offset, fetched, normalized, upserted, nizaLinks, total, lastActivityAt: new Date().toISOString() },
          },
        })
        .eq("id", run.id)
    ...
    await admin
      .from("inapi_sync_runs")
      .update({ status: "failed", finished_at: new Date().toISOString(), error_message: error instanceof Error ? error.message : String(error) })
      .eq("id", run.id)
    throw error
  }
}
...
async function upsertBatch(admin: ReturnType<typeof createAdminClient>, rows: NormalizedRow[]) {
  const payload = rows.map(({ niza: _niza, ...row }) => row)
  ...
  if (links.length) {
    const { error: linkError } = await admin
      .from("trademark_record_niza")
      .upsert(links, { onConflict: "trademark_record_id,code", ignoreDuplicates: true })
...
async function ckan(action: string, params: Record<string, string | number>) {
  ...
  const response = await fetch(url, {
    cache: "no-store",
    headers: { Accept: "application/json", "User-Agent": "VisualCompare-INAPI-Cron/1.0" },
  })
...
function normalizeOpenDataRow(raw: Record<string, unknown>, dataset: string): NormalizedRow | null {
  ...
  return {
    ...
    metadata: {
      openData: true,
      dataset,
      lastUpdatedDate: text(raw.LastUpdatedDate),
      signType: text(raw.SignType),
      typeName: text(raw.TypeName),
      subtypeName: text(raw.SubtypeName),
      viennaClasses: text(raw.VienaClasses),
