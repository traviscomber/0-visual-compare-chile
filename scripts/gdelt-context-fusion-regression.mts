...
const rawIndex = route.indexOf("syncGdeltRawFeed")
const contextIndex = route.indexOf("syncGdeltContextBundle", rawIndex)
const fusionIndex = route.indexOf("fuseGdeltIntoStrategicWatches", contextIndex)
assert.ok(rawIndex >= 0 && rawIndex < contextIndex, "Events must precede context")
assert.ok(contextIndex >= 0 && contextIndex < fusionIndex, "context must precede watch fusion")
assert.match(route, /let partial = false/, "cron must explicitly track degraded enrichment")
assert.match(route, /if \(!isContextBundleNotReady\(message\)\) throw error/, "unexpected context failures must remain blocking")
assert.match(route, /reason: "bundle_not_ready"/, "provider bundle lag must be represented as a bounded partial state")
assert.match(route, /reason: "post_processing_unavailable"/, "watch-fusion failure must be represented as non-blocking post-processing")
assert.match(route, /synchronized context bundle not ready; canonical raw ingest preserved/, "bundle lag must remain observable without invalidating raw ingest")
assert.match(route, /watch fusion deferred; canonical source corpus remains valid/, "watch-fusion deferral must remain observable")
assert.match(route, /ok: true,[\s\S]*partial,[\s\S]*events,[\s\S]*context,[\s\S]*watchFusion/, "successful raw ingestion must return its partial enrichment state")
assert.match(route, /canonical sync failed/, "true canonical failures must remain visible as failures")
assert.doesNotMatch(route, /console\.error\("\[cron\/gdelt-raw-feed\] sync failed"/, "enrichment lag must not be logged as a canonical sync failure")
