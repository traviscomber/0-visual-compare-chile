...
for (const needle of ['type SearchScope = "chile" | "global" | "both"', 'aria-label="Dónde buscar"', '<option value="chile">Chile</option>', '<option value="global">Global</option>', '<option value="both">Ambos</option>', 'type==="technology"?{scope}:{}', 'scopeLabel(watch.searchScope)', 'Inteligencia']) requireText(commonWatchesPage, needle, "common Watches UI")
