...
for (const needle of ["companyFit(haystack, intent, profile)", "intent.canonicalQuery", "...intent.aliases", "...intent.concept.core", "...intent.concept.context"]) requireText(qualitySource, needle, "bilingual company-fit bridge")
...
console.log("Search intent regression PASS: AI/IA equivalents share one concept identity, source scope stays explicit, Search Intent V2 remains auditable, company fit reuses the bilingual semantic intent surface, and every strategic event writer uses a canonical monotonic persistence path.")
