...
const [adapter, complianceSource, proceedingSource, provisionalSource, entryRequirementSource, cron, interactive, strategicScanner] = await Promise.all([
  ...
  readFile("lib/intelligence/snifa-provisional-measures.ts", "utf8"),
  readFile("lib/intelligence/snifa-entry-requirements.ts", "utf8"),
...
for (const needle of [
  ...
  "associatedProceedings",
]) requireText(provisionalSource, needle, "SNIFA provisional measure source")

for (const needle of [
  'const RESULTS_PATH = "/RequerimientoIngreso/Resultado"',
  "/RequerimientoIngreso\\/Ficha\\/(\\d+)",
  "normalizeEntity(item.holderName).includes(normalizedQuery)",
  "Sancionatorios asociados",
  "DFZ-",
  "fiscalizationCount",
  "latestActivityAt",
]) requireText(entryRequirementSource, needle, "SNIFA entry requirement source")
...
console.log("SNIFA canonical company regression PASS: official environmental sanctions, provisional measures, SEIA entry requirements, active sanctioning proceedings and compliance-program early warnings require one canonical company with verified RUT and source-holder corroboration; both persistence paths replace legacy text-only candidates and the shared strategic scanner performs no parallel SNIFA network scan.")
