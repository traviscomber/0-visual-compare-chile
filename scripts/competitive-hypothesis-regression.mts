...
for(const needle of [
  "requireUser()",
  'REVIEW_DECISIONS = new Set(["reviewed", "dismissed"])',
  '.eq("user_id", auth.user.id)',
  '.eq("review_status", "pending")',
  "next_review_at: nextReviewAt",
  "resolveLinkedExecutiveAction",
  'const sourceId = `${REVIEW_SOURCE_PREFIX}${eventId}`',
  '.from("case_items")',
  '.from("case_actions")',
  '.update({ status: "done", outcome',
  'review_status: "pending", review_reason: null, reviewed_by: null, reviewed_at: null, next_review_at: null',
  "La señal permanece pendiente",
])requireText(monitorApi,needle,"monitoring review API")
for(const forbidden of ["service_role","conviction_delta","confidence_delta","competitive_hypotheses\").update"])forbid(monitorApi,forbidden,"monitoring review API")
