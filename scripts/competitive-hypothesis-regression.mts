...
const [migration,route,page,monitorMigration,monitorLib,monitorCron,monitorApi,activeApi,activePage,vercel]=await Promise.all([
...
for(const needle of [
  ...
  "stale_review_due",
  "no_material_change",
  "review_status in ('pending','reviewed','dismissed','not_required')",
  "revoke all on table public.competitive_hypothesis_monitoring_events from anon, authenticated",
])requireText(monitorMigration,needle,"monitoring migration")
...
for(const needle of ["classifyContradictoryTitle","assessHypothesisMonitoring","input.contradictoryEvidence.length","newEvidence.length","unavailable.length","ageDays >= 90","no la vuelve falsa ni obsoleta"])requireText(monitorLib,needle,"monitoring classifier")
for(const needle of [
  'request.headers.get("authorization") !== `Bearer ${secret}`',
  '.eq("status", "accepted")',
  "DUE_AFTER_MS",
  "gatherExternalExpansionCorroboration",
  "gatherLocalPatentEvidence",
  "gatherContradictoryWebEvidence",
  "assessHypothesisMonitoring",
  '.from("competitive_hypothesis_monitoring_events").insert',
  'review_status: material ? "pending" : "not_required"',
])requireText(monitorCron,needle,"monitoring cron")
...
for(const needle of ["requireUser()","REVIEW_DECISIONS = new Set([\"reviewed\", \"dismissed\"])",'.eq("user_id", auth.user.id)','.eq("review_status", "pending")',"reviewed_by: auth.user.id","reviewed_at: reviewedAt"])requireText(monitorApi,needle,"monitoring review API")
for(const needle of ['.eq("status", "accepted")',"pendingReview","contradictory","stale","latestMonitoring"])requireText(activeApi,needle,"active hypotheses API")
for(const needle of ["Hipótesis competitivas activas","Nueva evidencia compatible","Señal contradictoria","Revisión por antigüedad","Registrar revisión","Descartar señal","no modifica conviction, score, prioridad, estado de oportunidad ni la aceptación original"])requireText(activePage,needle,"active hypotheses UI")
...
console.log("Competitive hypothesis regression PASS: corroborated Nice expansions require human acceptance before becoming active hypotheses; accepted hypotheses are monitored daily for new supporting evidence, explicit contradictory signals, source degradation and staleness; every material change returns to human review; and monitoring never mutates conviction, opportunity lifecycle, source evidence or the original human acceptance.")
