...
for(const needle of ["Seguimiento WIPO.","Escribe qué seguir","VIDENTIA lo conecta","Recibe novedades","Activar seguimiento","No necesitas crear enlaces, copiar RSS ni validar fuentes","VIDENTIA_WIPO_CONNECT",'params.get("feedUrl")','params.get("nonce")',"window.crypto.randomUUID()","window.localStorage.setItem","readPendingAutomaticConnection","window.localStorage.removeItem",'href="/monitorear"',"Revisar ahora","Tus seguimientos","Seguimiento activo","Pausar","Reactivar","Eliminar","Detalles técnicos",'/patentes/wipo/conector'])requireText(page,needle,"automatic WIPO UI")
for(const forbidden of ["Pega el RSS","Pega aquí la URL RSS","Validar RSS","navigator.clipboard.writeText","WIPO_SAVED_QUERIES_URL",'params.get("query")','params.get("type")'])forbidText(page,forbidden,"automatic WIPO UI")

for(const needle of ['"version": "0.3.0"','"storage", "tabs", "webRequest"','"https://patentscope.wipo.int/*"','"https://videntia.app/*"'])requireText(manifest,needle,"WIPO connector manifest")
for(const forbidden of ['"cookies"','"history"','"webRequestBlocking"'])forbidText(manifest,forbidden,"WIPO connector permissions")
for(const needle of ["WIPO_SAVED_QUERIES_URL","OPERATIONS_KEY","PENDING_TTL_MS","15 * 60 * 1000","isLiveOperation","findOperationByTabId","wipoTabId","about:blank","sender.tab?.id","details.tabId","chrome.webRequest.onBeforeRequest","parsed.hostname !== \"patentscope.wipo.int\"",'/\\/rss\\.xml$/i',"callback.searchParams.set(\"nonce\"","feedUrl","CALLBACK_BASE","completing = new Set"])requireText(background,needle,"WIPO connector background")
...
for(const needle of ["VIDENTIA_WIPO_READY","VIDENTIA_WIPO_CONNECT","START_WIPO_CONNECT","event.origin !== window.location.origin","message.nonce","nonce.length < 20"])requireText(videntiaBridge,needle,"VIDENTIA connector bridge")
...
console.log("WIPO patent watch regression PASS: automatic setup is nonce-authenticated and isolated per WIPO tab, the product UI hides RSS setup, the backend remains the canonical RSS validator, baseline suppression and provenance remain intact, and no WIPO credentials are collected.")
