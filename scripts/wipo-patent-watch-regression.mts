...
for(const needle of ["PATENTSCOPE como fuente internacional observable","Private Query desmarcado","Validar RSS","Polling cada 6 h","No usa cookies de tu sesión","Sin inferencia jurídica","Consultar ahora",'method:"PUT"',"Baseline pendiente","Fuentes verificadas","Con checkpoint de polling","WIPO_SAVED_QUERIES_URL","Abrir Saved Queries en WIPO","URL RSS completa","Pega aquí la URL RSS completa generada por PATENTSCOPE","No uses una dirección abreviada","Copiar RSS","navigator.clipboard.writeText","Abrir RSS"])requireText(page,needle,"WIPO UI")
...
console.log("WIPO patent watch regression PASS: PATENTSCOPE integrates through official public saved-query RSS with bounded host validation, explicit provenance, explicit/copyable full RSS URL UX, shared scheduled/manual polling, first-scan baseline suppression, staged source-aware schema rollout, degraded-source semantics and no browser-session scraping or legal-status inference.")
