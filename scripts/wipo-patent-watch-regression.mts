...
console.log("WIPO patent watch regression PASS: connected sources are the primary operational UX, RSS plumbing stays hidden behind an explicit new-source configuration path, and PATENTSCOPE integration preserves official public RSS provenance, scheduled/manual polling, baseline suppression, degraded-source semantics and no browser-session scraping or legal-status inference.")
