...
const [migration, profileApi, analyzerApi, onboardingPage, onboardingUi, appLayout, server, watchlistApi, watchSignalsApi, watchWriter] = await Promise.all([
...
for (const needle of [
  "persistIntelligenceWatchEvents",
...
requireText(watchWriter, 'onConflict: "user_id,watch_id,signal_key"', "canonical strategic event persistence")
const persistIndex = watchSignalsApi.indexOf("await persistIntelligenceWatchEvents(auth.supabase, rows)")
const completedAtIndex = watchSignalsApi.indexOf("const scanCompletedAt = new Date().toISOString()")
if (persistIndex < 0 || completedAtIndex < 0 || completedAtIndex <= persistIndex) {
  fail("strategic watch baseline timestamp must be captured after canonical event persistence")
}
