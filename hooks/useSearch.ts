...
export function useSearch() {
  ...
  const search = useCallback(
    async (params: SearchParams): Promise<SearchResponse> => {
      try {
        ...
        if (params.filters?.estado) url.searchParams.set('estado', params.filters.estado)
        if (params.filters?.pais) url.searchParams.set('pais', params.filters.pais)
        if (params.filters?.fechaDesde) url.searchParams.set('fechaDesde', params.filters.fechaDesde)
        if (params.filters?.fechaHasta) url.searchParams.set('fechaHasta', params.filters.fechaHasta)
        if (params.filters?.niza?.length) url.searchParams.set('niza', params.filters.niza.join(','))
        if (params.filters?.viena?.length) url.searchParams.set('viena', params.filters.viena.join(','))

        const apiResponse = await fetch(url.toString())
        const payload = apiResponse.ok ? await apiResponse.json() : null

        if (!apiResponse.ok || !payload) throw new Error('Error consultando la búsqueda de VIDENTIA')
        ...
        const tiempo_ms = typeof payload.tiempo_ms === 'number' ? payload.tiempo_ms : Math.round(performance.now() - startedAt)

        registrarBusqueda({ query: params.query, tipo: params.type, resultados: total })
        setResultados(results)
        setUltimaBusqueda(params)

        return { resultados: results, total, pagina: page, totalPaginas, tiempo_ms }
      } catch (err) {
        const mensaje = err instanceof Error ? err.message : 'Error desconocido'
        setError(mensaje)
        console.error('[videntia] Error en búsqueda:', err)
  ...
  const getStats = useCallback(async (): Promise<SearchStats | null> => {
    try {
      ...
      if (!response.ok || !payload || typeof payload.totalRecords !== 'number') return null
      ...
      console.error('[videntia] Error obteniendo estadísticas de búsqueda:', err)
      return null
    }
  }, [])
